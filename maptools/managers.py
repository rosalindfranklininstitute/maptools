...
def edit(args):
    ...
    with models.MapFile(
        ...
        if args.output is not None:
            with models.MapFile(
                args.output, "w", colour=args.colour, verbose=args.verbose
            ) as mapout:
                mapout.copy(mapin)
...
def create(args):
    ...
    with models.MapFile(
        ...
        if args.verbose:
            print(
                Styled(
                    f"[[ '[info] creating map with {args.voxel_values}'|fg-orange_3 ]]"
                ),
                file=sys.stderr,
            )
            print(
                Styled(f"[[ '[info] creating map mode {args.map_mode}'|fg-orange_3 ]]"),
                file=sys.stderr,
            )
