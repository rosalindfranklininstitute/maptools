...
# options
file = {"args": ["file"], "kwargs": dict(help="a valid EMDB MAP file")}
orientation = {
    "args": ["-O", "--orientation"],
    "kwargs": dict(
        type=lambda s: s.upper(),
        choices=list(
            map(lambda t: "".join(t), itertools.permutations("XYZ", 3))
        ),  # all 3-permutations
...
output = {"args": ["-o", "--output"], "kwargs": dict(help="output file name")}

label = {
    "args": ["-l", "--label"],
    "kwargs": dict(help="label to insert; will be truncated at 80 chars"),
}

parser = argparse.ArgumentParser(
    prog="map", description="Utilities to work with EMDB MAP files"
)
...
output_mutex_parser.add_argument(
    "-q",
    "--quiet",
    default=True,
    action="store_true",
...
_add_arg(
    edit_parser,
    label,
    default=f"{datetime.datetime.now().strftime('%d/%m/%y %H:%M')} - edited with maptools",
)
...
_add_arg(
    create_parser,
    label,
    default=f"{datetime.datetime.now().strftime('%d/%m/%y %H:%M')} - created with maptools",
)
...
sample_parser.add_argument(
    ...
    type=int,
    default=2,
    choices=SAMPLE_FACTORS,
    help="sampling factor [default=2]",
)
